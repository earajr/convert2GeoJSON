...
def main():
    ...
    input_file, output_dir, VAR_NAME, SOURCE, smooth_dict, contour_dict, level_dict, parallel_dict = utils.input_args()

    print (input_file, output_dir, VAR_NAME, SOURCE, smooth_dict, contour_dict, level_dict, parallel_dict)

    # Read data file

#    data = read_data.read_datafile(SOURCE, input_file, VAR_NAME, contour_dict, level_dict)
    ...
    # Loop over entries in data and smooth data if required
        ...
        # Get features
            feature_collection = contouring_par.generate_geojson(var=data[entry]['values'], lat=data[entry]['lat'], lon=data[entry]['lon'], contours=LEVELS, thresholds=THRESHOLDS, metadata=data[entry]['metadata'], hex_palette=hex_palette, max_workers=parallel_dict["max_workers"])
        ...
        # Save output
            contouring.write_geojson(feature_collection, output_dir, input_file, entry, VAR_NAME, data[entry]['metadata'])
