...
from shapely.geometry import Polygon, mapping, MultiPolygon, LineString, LinearRing, shape
...
def plot_raw_polygons(raw_polygons, title="Raw polygons"):
    fig, ax = plt.subplots(figsize=(8, 8))

    for idx, polys in raw_polygons.items():
        for poly in polys:
...
def get_category_feature_data(var, lat, lon, thresholds):
    """
    ...
    # ---- Handle masked arrays correctly ----
    ...
    # ---- Bin integer values using half-integer thresholds ----
    # thresholds define bin edges
    categories = np.digitize(values, thresholds) - 1

    # mark invalid
    categories[~mask] = -1

    data = categories.astype(np.int32)
    mask = data >= 0

    # ---- Build affine (regular grid) ----
    dx = dy = 0.1
    ...
    raw_polygons = {i: [] for i in range(len(thresholds) - 1)}

    for geom, value in shapes(data, mask=mask, transform=transform):
        raw_polygons[int(value)].append(shape(geom))
...
def process_tile(args):
    ...
    Returns:
    --------
    raw_polygons: Polygons that have been created using get_contour_feature_data
    ...
    if tile_bbox_with_overlap is None or tile_bbox_core is None:
        ...
        try:
            raw_polygons = get_category_feature_data(var_tile, lat_tile, lon_tile, thresholds)
...
#    plt.imshow(
#        var,
#        origin="lower",  # IMPORTANT: matches your affine orientation
#        extent=[
#            lon.min(), lon.max(),
#            lat.min(), lat.max()
#        ],
